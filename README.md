...
### `policy.approvalRules` (Diff + Role + Environment)

Approval rules govern control-plane YAML mutations generated by DSL compilation.

| Field | Required | Description |
|---|---|---|
| `id` | ✔ | Unique approval-policy rule id. |
| `match.path` | ✔ | YAML diff path selector (supports `*` wildcard root). |
| `match.operation` | ✔ | `"add"` · `"remove"` · `"update"` |
| `scope.roles` | | Optional role scope: `architect` · `analyst` · `conductor` · `enforcer` |
| `scope.environments` | | Optional environment scope: `local` · `ci` · `staging` · `production` |
| `condition.contains` | | Optional substring condition on diff source value. |
| `condition.countGreaterThan` | | Optional size/count threshold condition. |
| `effect.type` | ✔ | `"allow"` · `"require-approval"` · `"deny"` |
| `effect.message` | | Optional message for deny/approval traces. |
...
- Policy traces include role, environment, matched rules, and final decision for auditability
...
policy:
  ...
  approvalRules:
    - id: block-structural-changes-prod
      ...
      effect:
        type: deny
        message: Cannot modify execution plans in production
    ...
    - id: analyst-read-only
      ...
      effect:
        type: deny
        message: Analyst role is read-only for YAML mutations
...
- Identical `(yamlDiff, role, environment, policySet)` inputs always produce identical policy decisions
