...
export type VerifyChatCommand = {
  type: "verify";
  mode: "full" | "quick" | "property" | "chaos" | "contracts" | "determinism" | "transactions" | "state" | "policy" | "orchestration" | "compiler";
...
export function parseVerifyChatCommand(input: string): VerifyChatCommand | null {
  ...
  if (/^(?:@choir\s+)?verify\s+--transactions\s*$/i.test(normalized)) {
    return {
      type: "verify",
      mode: "transactions",
    };
  ...
  if (/^(?:@choir\s+)?verify\s+--state\s*$/i.test(normalized)) {
    return {
      type: "verify",
      mode: "state",
    };
  ...
  if (/^(?:@choir\s+)?verify\s+--policy\s*$/i.test(normalized)) {
    return {
      type: "verify",
      mode: "policy",
    };
  ...
  if (/^(?:@choir\s+)?verify\s+--orchestration\s*$/i.test(normalized)) {
    return {
      type: "verify",
      mode: "orchestration",
    };
  ...
  if (/^(?:@choir\s+)?verify\s+--compiler\s*$/i.test(normalized)) {
    return {
      type: "verify",
      mode: "compiler",
    };
